...
const planned = JSON.parse(readFileSync(join(ROOT, 'design/backlog.json'), 'utf8')).icons.find((i) => i.name === name);
...
writeFileSync(join(ICON_DIR, `${name}.json`), JSON.stringify({ tags: name.split('-'), categories: [cat], contributors: [] }, null, 2) + '\n');
say(`icons/${name}.svg and .json written${planned ? ` (backlog: ${planned.concept})` : ''}. Draw it, then npm run check.`);
