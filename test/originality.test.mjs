...
test('the originality check catches a pasted Lucide icon', { skip: !lucideDir() && 'lucide-static not installed' }, async () => {
  const { parseIcon, elementPoints } = await import('../scripts/lib.mjs');
  const sig = (svg) => new Set(parseIcon(svg).children.map((c) => `${c[0]}:${elementPoints(c).map(([x, y]) => `${Math.round(x * 2) / 2},${Math.round(y * 2) / 2}`).join(' ')}`));
