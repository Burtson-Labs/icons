...
const HEAD = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">';
const META = { tags: ['a', 'b'], categories: ['security'] };
const check = (body, meta = META, name = 'probe') => validateIcon(name, `${HEAD}${body}</svg>`, meta, readCategories()).errors;

test('every icon in the repo validates', () => {
  const failing = validateAll().filter((r) => r.errors.length);
  assert.deepEqual(failing.map((r) => `${r.name}: ${r.errors.join('; ')}`), []);
});

test('the validator rejects what the spec forbids', () => {
  assert.equal(check('<circle cx="12" cy="12" r="9" />').length, 0);
  assert.match(check('<circle cx="12" cy="12" r="9" fill="red" />').join(), /styling belongs to the root/);
  ...
  assert.match(check('<circle cx="12" cy="12" r="9" />', { tags: ['a'], categories: ['security'] }).join(), /2 search tags/);
  assert.match(check('<circle cx="12" cy="12" r="9" />', { tags: ['a', 'b'], categories: ['nope'] }).join(), /unknown category/);
  assert.match(check('<circle cx="12" cy="12" r="9" />', META, 'Bad_Name').join(), /kebab-case/);
  const wrongRoot = validateIcon('probe', HEAD.replace('stroke-width="2"', 'stroke-width="1.5"') + '<circle cx="12" cy="12" r="9" /></svg>', META, readCategories());
